...
model = nude_detector.load_model("/home/taruu/PycharmProjects/nude-check-tests/models/nsfw_models/mobilenet_v2_140_224/saved_model.h5")

for image_path in list_images_to_test:
    with open(image_path, 'rb') as image_file:
        image_bytes = image_file.read()
        caption, tag_list, ratings, character, general = wdv3_Worker.get_image_marks(io.BytesIO(image_bytes))
        result = nude_detector.classify(model, io.BytesIO(image_bytes))
        print()
        print(image_path)
        print(ratings)
        print(result)
